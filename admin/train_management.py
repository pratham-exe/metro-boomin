...
for item in trains:
    ...
    with col1:
        st.write(item[0])
    with col2:
        result = session.execute(text(f"SELECT station_ids FROM route WHERE route_id = :route_id"), {'route_id': item[1]})
        stations = result.scalar().split(',')
        route_start_id = stations[0]
        route_end_id = stations[-1]
        route_start_name = session.execute(text(f"SELECT station_name FROM station WHERE station_id = :station_start"), {'station_start': route_start_id}).scalar()
        route_end_name = session.execute(text(f"SELECT station_name FROM station WHERE station_id = :station_end"), {'station_end': route_end_id}).scalar()
